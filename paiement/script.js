...
//AngularJS
var majApp = angular.module('majApp', ['AxelSoft','angularjs-datetime-picker']);
...
function func_majCtrl($scope, $http) {
	...
	req.then(function mySuccess(response) {
			console.log("fetch_combo_propriete");
			console.log(response);
			$scope.proprietes = response.data;
		},
	...
	//If exist populate form else let them empty
	var the_id;
	$.urlParam = function(name) {
		var results = new RegExp('[\?&]' + name + '=([^&#]*)').exec(window.location.href);
		if (results == null)
...
// 				$("#date_paiement").datepicker("setDate", response.data[0].date_paiement);
				...
				$scope.propriete = response.data[0].num_propriete;
			},
			function myError(response) {
...
$("#table_paiement").tabulator({
	...
	columns: [ //Define Table Columns
		...
		{
			title: "Affectation paiement",
			field: "affectation_paiement",
			headerFilter: true
		},
		{
