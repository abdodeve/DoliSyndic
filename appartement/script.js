...
function func_majCtrl ($scope,$http) {
    ...
    //Default
    $scope.titre_page = 'Nouveau appartement' ;
    ...
    if(the_id != null)
    {
        $scope.titre_page = 'Modifier appartement' ;
        var ajaxUrl = window.location.origin+"/htdocs/syndic/appartement/class/appartementHandler.class.php" ;
        var req = $http({
            method  :   'POST',
            url     :   ajaxUrl,
        ...
        req.then(function mySuccess(response) {
                ...
                $scope.num_appartement      = response.data[0].num_appartement;
        ...
        if (the_id != null) {
            ...
            var req = $http({
                ...
                data: {
                        action              : "update",
                        id                  : the_id,
                        num_appartement     : $scope.num_appartement,
...
$("#table_appartement").tabulator({
    ...
    columns:[ //Define Table Columns
        {title:"Num appartement", field:"num_appartement", headerFilter:true},
...
$("#table_appartement").tabulator("setLocale", "fr-fr");
...
function func_listeCtrl ($scope,$http) {
    ...
    function load() {
        ...
        req.then(function mySuccess(response) {
                ...
                console.log('Appartement Adev Data');
                console.log(tabledata);
                //load data into the table
                $("#table_appartement").tabulator("setData", tabledata);
            },
            function myError(response) {
    ...
    $scope.confirm_func = function () {
        ...
        var listChecked = [];
        var listAll = $("#table_appartement").tabulator("getData");
        ...
        req.then(function mySuccess(response) {
                ...
                angular.forEach(listChecked, function (value, key) {
                    $("#table_appartement").tabulator("deleteRow", value);
                });
    ...
    $scope.select_all_func = function () {
        var dataUpdate = [];
        var productData = $("#table_appartement").tabulator("getData");

        if ($('#check-all').prop("checked")) {
            $.each(productData, function (i, item) {
                var obj = {
                    "id"                  : item.id,
                    "num_appartement"     : item.num_appartement,
        ...
        $("#table_appartement").tabulator("updateData", dataUpdate);
    };
}
